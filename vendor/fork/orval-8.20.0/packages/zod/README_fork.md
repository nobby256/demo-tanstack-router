...
このディレクトリは、`orval@8.20.0` に含まれる `@orval/zod` をベースにしたローカル改造版です。

本実装は Orval 8.20 系の `zod` generator に対する拡張です。

なお、本拡張は Zod Classic の生成方式を前提として実装されており、Orval 8.20 で追加された Zod Mini (`variant: "mini"`) には対応していません。
...
本実装は Orval の Zod Classic 生成方式を対象としています。
...
または variant 未指定時のデフォルト設定で利用してください。
...
Zod Mini は `.check(...)` ベースの別レンダラを使用します。

本拡張は classic のメソッドチェーン生成を前提として実装されているため、`variant: "mini"` では利用できません。
